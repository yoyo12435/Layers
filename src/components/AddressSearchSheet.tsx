    ...
    <div className="absolute inset-0 z-[700] flex flex-col items-center pt-20 px-4">
      ...
      <div className="relative w-full max-w-sm bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl max-h-[70vh] flex flex-col">
        ...
        {activeLayerName === null ? (
          <p className="px-4 pb-6 text-sm text-neutral-500 dark:text-neutral-400">
          ...
          <>
            <p className="px-4 pb-2 text-xs text-neutral-400">
              Adding to <span className="font-medium text-neutral-600 dark:text-neutral-300">{activeLayerName}</span>
            </p>
            ...
            <div className="px-4 pb-3">
              <button
                ...
                disabled={locating}
                className="w-full flex items-center gap-2.5 border border-neutral-200 dark:border-neutral-700 rounded-xl px-3 py-2.5 mb-2 text-sm font-medium text-blue-600 dark:text-blue-400 hover:bg-neutral-50 dark:hover:bg-neutral-800"
              >
                <LocateIcon className={`w-4 h-4 shrink-0 ${locating ? 'animate-spin' : ''}`} />
                {locating ? 'Finding your location…' : 'Use current location'}
              </button>

              <div className="flex items-center gap-2 border border-neutral-300 dark:border-neutral-700 rounded-xl px-3 py-2.5">
                ...
                <input
                  ...
                  className="flex-1 min-w-0 text-sm bg-transparent text-neutral-900 dark:text-white focus:outline-none"
                />
              </div>
            </div>
              ...
              <ul className="divide-y divide-neutral-100 dark:divide-neutral-800">
                {results.map((r, i) => (
                  <li key={i}>
                    ...
                    >
                      <p className="text-sm font-medium text-neutral-900 dark:text-white truncate">{r.shortName}</p>
